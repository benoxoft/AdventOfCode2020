input = """shiny indigo bags contain 4 vibrant lime bags.
...
for rule in rules:
    ...
    container, rest = rule.split(" contain ")
    contains = rest.split(", ")
    bag_rules.setdefault(container, {})
    for bags in contains:
        if bags == "no other bag":
            continue
        amount = int(bags[0])
        bag = bags[2:]
        bag_rules[container][bag] = amount

def find_shiny_bags(bag_name):
    shiny_bag_holders = 0
    content = bag_rules[bag_name]
    for inner_bag_name in content.keys():
        if inner_bag_name == "shiny gold bag":
            shiny_bag_holders += 1
        shiny_bag_holders += find_shiny_bags(inner_bag_name)
    return shiny_bag_holders

def enumerate_shiny_bags_content(bag_name, parent_bag_amount):
    bag_amount = 0
    content = bag_rules[bag_name]
    for inner_bag_name, amount in content.items():
        bag_amount += amount * parent_bag_amount
        bag_amount += enumerate_shiny_bags_content(inner_bag_name, parent_bag_amount * amount)
    return bag_amount

shinies = 0
for bag_name in bag_rules.keys():
    if find_shiny_bags(bag_name) > 0:
        shinies += 1
print("part1", shinies)

print("part2", enumerate_shiny_bags_content("shiny gold bag", 1))
