input = """LLLLLL.LLLLLLLLLLLL.LLLLLL.LLLLLLLLLLLL.LLLLLLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLL.LLLLLLLLL
...
class Seat:
    def __init__(self, postype) -> None:
        if postype == ".":
            raise Exception("Cannot sit on the floor")
        self.postype = postype
        self.newposttype = postype
        self.adjacents = []
    def is_seat(self):
        return True
    def link(self, pos):
        self.adjacents.append(pos)
    ...
    def update_new_pos(self):
        ...
        if occ >= 5:
            ...
            self.newposttype = self.postype
    def commit_new_pos(self):
        self.postype = self.newposttype
    def has_changes(self):
        return self.newposttype != self.postype

class Floor:
    def __init__(self, postype) -> None:
        if postype != ".":
            raise Exception("Cannot sit on the floor")
    def is_seat(self): return False
    def link(self, pos): pass
    def commit_new_pos(self): return
    def has_changes(self): return False
    def is_occupied(self): return False
    def update_new_pos(self): return
...
for i in range(0, rows_len):
    ...
    for j in range(0, row_len):
        ...
        loose_pos.append(pos)

assert len(positions) == rows_len
assert len(positions[0]) == row_len

def link_part2():
    def link_pos(i, j, incx, incy, pos, positions):
        idx = incx
        idy = incy
        while True:
            try:
                if i+idx < 0 or j+idy < 0:
                    break
                if positions[i+idx][j+idy].is_seat():
                    pos.link(positions[i+idx][j+idy])
                    break
                else:
                    idx += incx
                    idy += incy
    ...
    for i in range(0, rows_len):
        for j in range(0, row_len):
            pos = positions[i][j]
            link_pos(i, j, -1, 0, pos, positions)    
            link_pos(i, j, -1, -1, pos, positions)    
            link_pos(i, j, -1, 1, pos, positions)    
            link_pos(i, j, 0, -1, pos, positions)    
            link_pos(i, j, 0, 1, pos, positions)    
            link_pos(i, j, 1, 0, pos, positions)    
            link_pos(i, j, 1, 1, pos, positions)    
            link_pos(i, j, 1, -1, pos, positions)    
...
def link_part1():
    for i in range(0, rows_len):
        for j in range(0, row_len):
            pos = positions[i][j]
            if i-1 >= 0:
                pos.link(positions[i-1][j])
            if i-1 >= 0 and j-1 >= 0:
                pos.link(positions[i-1][j-1])
            if i-1 >= 0 and j+1 < row_len:
                pos.link(positions[i-1][j+1])
            if j-1 >= 0:
                pos.link(positions[i][j-1])
            if j+1 < row_len:
                pos.link(positions[i][j+1])
            if i+1 < rows_len:
                pos.link(positions[i+1][j])
            if i+1 < rows_len and j+1 < row_len:
                pos.link(positions[i+1][j+1])
            if i+1 < rows_len and j-1 >=0:
                pos.link(positions[i+1][j-1])
...
while True:
    has_changes = False
    for pos in loose_pos:
        pos.update_new_pos()
    for pos in loose_pos:
        if pos.has_changes():
            has_changes = True
    if not has_changes:
        print("CHAOS STABILIZED", counter)
        seats = 0
        for pos in loose_pos:
            if pos.is_occupied():
                seats += 1
