input = """hcl:#6b5442 ecl:brn iyr:2019
...
def validate(passport):
    try:
        ...
        if "cm" in passport["hgt"]:
            ...
            raise Exception()
        assert "hcl" in passport.keys()
        assert passport["hcl"][0] == "#"
        assert passport["hcl"][1] in "abcdef1234567890"
        assert passport["hcl"][2] in "abcdef1234567890"
        assert passport["hcl"][3] in "abcdef1234567890"
        assert passport["hcl"][4] in "abcdef1234567890"
        assert passport["hcl"][5] in "abcdef1234567890"
        assert passport["hcl"][6] in "abcdef1234567890"
        assert len(passport["hcl"]) == 7

        assert "ecl" in passport.keys()
        assert passport["ecl"]in ("amb", "blu", "brn", "gry", "grn", "hzl", "oth")

        assert "pid" in passport.keys()
        assert len(passport["pid"]) == 9
        int(passport["pid"])
        
        assert "cid" in passport.keys() or True
        return 1
    except:
        return 0
...
for line in input.split("\n"):
    if line == "":
        valid_passports += validate(passport)
        passport = {}
    else:
        tokens = line.split(" ")
        for token in tokens:
